...
bool _shouldExcludeFromGate(String sourcePath) {
  ...
  // Large visual chrome — covered by smoke/widget pumps, not line-dense unit tests.
  if (normalized.endsWith('family_membership_badge.dart')) return true;
  if (normalized.endsWith('family_auth_chrome.dart')) return true;
  if (normalized.endsWith('family_plans_chrome.dart')) return true;
  if (normalized.endsWith('family_chrome.dart')) return true;
  if (normalized.endsWith('family_shell_header.dart')) return true;
  return false;
}
